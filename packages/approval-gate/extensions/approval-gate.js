...
import {
  ...
  isStaticallyAllowed,
  loadPolicy,
} from "../approval-gate/lib/policy.js";
import {
  canRememberCommand,
  clearApprovals,
  cwdKey,
  ...
  purgeApprovals,
  targetsPath,
} from "../approval-gate/lib/store.js";
...
export default function approvalGate(pi) {
  ...
  pi.on("session_start", async (_event, ctx) => {
    try {
      ...
      ctx.ui.notify(
        `Approval policy could not be loaded: ${message(error)}`,
        "error",
      );
    }
    try {
      ...
      approvals.clear();
      ctx.ui.notify(
        `Command approvals could not be loaded: ${message(error)}`,
        "error",
      );
    }
  });
  ...
  pi.on("tool_call", async (event, ctx) => {
    ...
    const options =
      !oneTimeOnly && canRememberCommand(command)
        ? [deny, runOnce, alwaysAllow]
        : [deny, runOnce];
    const title = oneTimeOnly
      ? "Harmful command requires approval"
    ...
    if (choice === alwaysAllow && !oneTimeOnly) {
      try {
        ...
        ctx.ui.notify(
          `Approval could not be saved: ${message(error)}`,
          "error",
        );
        return {
          block: true,
    ...
    handler: async (args, ctx) => {
      ...
      if (action === "clear") {
        try {
          ...
          approvals.clear();
          ctx.ui.notify(
            "Approvals cleared for this working directory.",
            "info",
          );
        } catch (error) {
          ctx.ui.notify(
            `Approvals could not be cleared: ${message(error)}`,
            "error",
          );
        }
        return;
